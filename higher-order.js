...
let totalPopulation = populations.reduce((acc,curr) => {
  return acc + curr
},0); //  = populations.reduce(/* Provide Your Callback Here */)
...
//Code Here
let myStrongest = monstersInYourPocket.filter((ele) => {
  return ele.CP > 200
})
...
let orderTotals = orders.map(ele => {
  return ele.price + (ele.price * ele.tax)
})
